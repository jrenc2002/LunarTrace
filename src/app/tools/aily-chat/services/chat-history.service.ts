...
export class ChatHistoryService implements OnDestroy {
  ...
  /**
   * 从旧格式迁移历史数据（兼容 ChatService 的 .chat_history/.chat 格式）
   * 在初次加载时调用一次
   */
  migrateFromLegacy(projectPath: string): void {
    if (!this.hasFs()) return;

    try {
      const legacyIndexPath = this.joinPath(projectPath, this.PROJECT_CHAT_DIR, '.chat');
      if (!this.fileExists(legacyIndexPath)) return;

      const legacyIndex = JSON.parse(this.readFileSync(legacyIndexPath));
      ...
      for (const legacy of legacyIndex) {
        const { sessionId, name } = legacy;
        if (!sessionId) continue;

        // 跳过已迁移的
        if (this.index.find(e => e.sessionId === sessionId)) continue;

        // 读取旧格式聊天数据
        const legacyDataPath = this.joinPath(projectPath, this.PROJECT_CHAT_DIR, `${sessionId}.json`);
        let chatList: ChatListItem[] = [];
        if (this.fileExists(legacyDataPath)) {
          try {
            chatList = JSON.parse(this.readFileSync(legacyDataPath));
          } catch { }
        }
        ...
        this.writeSessionData(sessionId, sessionData);
        this.upsertIndexEntry(sessionId, sessionData.metadata, chatList.length);
        migratedCount++;
      }

      if (migratedCount > 0) {
        this.writeIndex();
        console.log(`[ChatHistory] 从旧格式迁移了 ${migratedCount} 条历史记录 (${projectPath})`);
      }
    } catch (error) {
      console.warn('[ChatHistory] 旧格式迁移失败:', error);
    }
  }
